      ...
      if show {
        VStack {
          ...
          HStack(spacing: Space.small) {
            Text(message)
              .typography(.body(color: CustomColor.inverseOnSurface))
            ...
            if withCloseIcon {
              Button {
                ...
                IconView(
                  icon: Icons.close,
                  color: CustomColor.inverseOnSurface)
              }
            }
          }
          .padding(Space.medium)
          .background(CustomColor.inverseSurface)
          .cornerRadius(RoundedShape.extraSmall)
        }
        .padding(
          EdgeInsets(
            top: Space.small,
